...
#include <time.h>
#include <cfloat>
// #include "fastexp.h"  ### commentouted by Shin Asakawa
#include "rnnlmlib.h"
...
void CRnnLM::computeNet(int last_word, int word)
{
	...
	if (neu1[a].ac>50) neu1[a].ac=50;  //for numerical stability
        if (neu1[a].ac<-50) neu1[a].ac=-50;  //for numerical stability
        val=-neu1[a].ac;
        // neu1[a].ac=1/(1+fasterexp(val));  ## commentouted by S.A.
	...
	for (a=0; a<layerc_size; a++) {
	    if (neuc[a].ac>50) neuc[a].ac=50;  //for numerical stability
    	    if (neuc[a].ac<-50) neuc[a].ac=-50;  //for numerical stability
    	    val=-neuc[a].ac;
    	    // neuc[a].ac=1/(1+fasterexp(val));  ## commentouted by S.A.
    ...
    for (a=vocab_size; a<layer2_size; a++)
        // sum+=fasterexp(neu2[a].ac-maxAc);  ## commentouted by S.A.
        sum+=exp(neu2[a].ac-maxAc);
    for (a=vocab_size; a<layer2_size; a++)
        // neu2[a].ac=fasterexp(neu2[a].ac-maxAc)/sum;  ## commentouted by S.A.
        neu2[a].ac=exp(neu2[a].ac-maxAc)/sum; 
